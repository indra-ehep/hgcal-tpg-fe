/**********************************************************************
 ...
 Purpose    : Match and study the features of VBF jets
...
int main(int argc, char** argv)
{
  ...
  bool doPrint = 0;
  std::cout << "========= Run as : ./vbfjet.exe $input_file $nofevents ==========" << std::endl;
  ...
  for (Long64_t ievent = 0 ; ievent < nofEvents ; ievent++ ) {    
    ...
    std::vector<int> inPartons, outPartons;
    for(int igen=0; igen<gen_n; igen++ ){
      ...
      if(gen_status->at(igen)==21) { //status 21 for incoming particles, 23 for outgoing particles
	inPartons.push_back(igen);
      }
      std::string daughterlist = "(";
      for(int const& idx: gen_daughters->at(igen)) {
	daughterlist += std::to_string(idx);
	daughterlist += ",";
	if(gen_status->at(igen)==21 and std::find(outPartons.begin(),outPartons.end(),idx)==outPartons.end() and gen_pdgid->at(idx)!=25 and gen_pdgid->at(idx)!=21 and gen_status->at(idx)==23) outPartons.push_back(idx);
      }
      daughterlist += ")";
      
      if(igen<8 and (ievent==3 or ievent==11 or ievent==17 or ievent==24))
    ...
    if(outPartons.size()!=2)
      std::cout << "ievent: " << std::setprecision(default_precision) << std::setw(4) <<ievent << ", inPartons.size(): " << inPartons.size() << ", outPartons.size(): " << outPartons.size() << std::endl;
