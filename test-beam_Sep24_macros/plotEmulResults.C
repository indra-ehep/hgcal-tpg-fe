...
int plotEmulResults(uint32_t relayNumber = 1726581356)
{
  ...
  TH1D *hUnpkWordDiff[4][2][3][10]; //2 modes(tctp==any or 1), 2 lpGBTs, 3 modules in each lpGBT 8 words max
  ...
  TH1D *hElinkDiff_2 = (TH1D *)fin->Get("diff_plots/hElinkDiff_2");
  TH1D *hElinkDiff_3 = (TH1D *)fin->Get("diff_plots/hElinkDiff_3");
  ...
  TH1D *hUWord_2 = (TH1D *)fin->Get("diff_plots/hUWord_2");
  TH1D *hUWord_3 = (TH1D *)fin->Get("diff_plots/hUWord_3");
  ...
  for(int itctp = 0; itctp <= 3 ; itctp += 2){
    for(int ilp=0;ilp<2;ilp++){
      for(int imdl=0;imdl<3;imdl++){
	std::string modtype = (ilp==0) ? "BestC" : "STC4A" ; 
	...
	///////Enegy////////
	std::string mname = Form("%u_07_Energy_tctp-%d_lpGBT-%d_module-%d",relayNumber,itctp,ilp,imdl);
	...
	/////////Address///////////
	mname = Form("%u_08_Address_tctp-%d_lpGBT-%d_module-%d",relayNumber,itctp,ilp,imdl);
