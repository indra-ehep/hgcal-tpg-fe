/**********************************************************************
 ...
 Purpose    : DQM plot of TPG data for 3 silicon trains, 1 motherboard and external trigger
...
int main(int argc, char** argv)
{
  ...
  void FillHistogram(TDirectory*&, uint32_t, uint64_t, uint16_t, uint32_t, uint32_t, uint32_t, TPGFEDataformat::TcRawDataPacket&);
  
  TFile *fout = new TFile(Form("DQM-TPGData_Plots_3T_Relay-%u.root",relayNumber), "recreate");
  TDirectory *dir_plot = fout->mkdir("plots");
  BookHistograms(dir_plot, relayNumber);
  //===============================================================================================================================
	...
	for(int iecont=0;iecont<maxecons;iecont++){
	  ...
	  for(const auto& econtit : econtdata){
		...
		std::cout << "========== end of TPG data for ibx : " << ibx << " ==========" << std::endl;
	      ...
	      int nTC = (ilink==0) ? econTPar[moduleId].getNofTCs() : econTPar[moduleId].getNofSTCs() ; 
	      FillHistogram(dir_plot, relayNumber, event, ibx, ilink, iecont, nTC, vTCel);
...
void BookHistograms(TDirectory*& dir_plot, uint32_t relay){
  
  TH1D *hBxEnergy[4][3];     //4 lpGBTs, 3 maximum modules in each lpGBT
  TH1D *hTCEnergy[4][3][48]; //4 lpGBTs, 3 maximum modules in each lpGBT, all TC addresses
  TH1D *hOccupancy[4][3];   //4 lpGBTs, 3 maximum modules in each lpGBT
  
  for(int ilp=0;ilp<4;ilp++)
    for(int imdl=0;imdl<3;imdl++){
      //------------------
      hBxEnergy[ilp][imdl] = new TH1D(Form("hBxEnergy_%d_%d",ilp,imdl), Form("hBxEnergy_%d_%d",ilp,imdl), 7, -3.5, 3.5);
      //hBxEnergy[ilp][imdl]->SetMinimum(1.e-1);
      hBxEnergy[ilp][imdl]->GetXaxis()->SetTitle("Bx");
      hBxEnergy[ilp][imdl]->GetYaxis()->SetTitle("Energy");      
      //---------------------
      int nTC = 0;
      if(ilp==0){
	nTC = 48 ;
	...
	nTC = (imdl==0) ? 12 : 10; 
      ...
      //--------------------
      hOccupancy[ilp][imdl] = new TH1D(Form("hOccupancy_%d_%d",ilp,imdl), Form("hOccupancy_%d_%d",ilp,imdl), 50, -0.5, 49.5);
      hOccupancy[ilp][imdl]->GetXaxis()->SetTitle("TC");
      hOccupancy[ilp][imdl]->GetYaxis()->SetTitle("Entries");
      //---------------------
      for(int itc=0;itc<nTC;itc++){
	hTCEnergy[ilp][imdl][itc] = new TH1D(Form("hTCEnergy_%d_%d_%d",ilp,imdl,itc), Form("hTCEnergy_%d_%d_%d",ilp,imdl,itc), 100, 0, 2000);
	hTCEnergy[ilp][imdl][itc]->GetXaxis()->SetTitle("Energy");
	hTCEnergy[ilp][imdl][itc]->GetYaxis()->SetTitle("Entries");
      }//itc loop
      //---------------------
    }//imdl loop
  ...
  for(int ilp=0;ilp<4;ilp++)
    for(int imdl=0;imdl<3;imdl++)
      hBxEnergy[ilp][imdl]->SetDirectory(dir_plot);
  
  for(int ilp=0;ilp<4;ilp++)
    for(int imdl=0;imdl<3;imdl++)
      hOccupancy[ilp][imdl]->SetDirectory(dir_plot);
  ...
  for(int ilp=0;ilp<4;ilp++)
	...
	nTC = (imdl==0) ? 12 : 10; 
      ...
      for(int itc=0;itc<nTC;itc++)
	hTCEnergy[ilp][imdl][itc]->SetDirectory(dir_plot);
    }
}

void FillHistogram(TDirectory*& dir_plot, uint32_t relayNumber, uint64_t event, uint16_t inputbxId,
		   uint32_t ilp, uint32_t imdl, uint32_t nTC, TPGFEDataformat::TcRawDataPacket& tcdata){
  
  
  TList *list = (TList *)dir_plot->GetList();
  ...
  if(ilp==0){
    ((TH1D *) list->FindObject(Form("hBxEnergy_%d_%d",ilp,imdl)))->Fill( (inputbxId-3), TPGFEDataformat::TcRawData::Decode5E3M(tcdata.moduleSum()) );
  }else{
    uint32_t totE = 0;
    for(int itc=0;itc<nTC;itc++) totE += tcdata.getTc(itc).decodedE(tcdata.type());
    ((TH1D *) list->FindObject(Form("hBxEnergy_%d_%d",ilp,imdl)))->Fill( (inputbxId-3), totE );
  }

  if(ilp==0){
    if(inputbxId==3)
       for(int itc=0;itc<nTC;itc++) ((TH1D *) list->FindObject(Form("hOccupancy_%d_%d",ilp,imdl)))->Fill( tcdata.getTc(itc).address() );
  }else if (ilp==1){
    for(int itc=0;itc<nTC;itc++) ((TH1D *) list->FindObject(Form("hOccupancy_%d_%d",ilp,imdl)))->Fill( itc*16 + tcdata.getTc(itc).address() );
  }else{
    for(int itc=0;itc<nTC;itc++) ((TH1D *) list->FindObject(Form("hOccupancy_%d_%d",ilp,imdl)))->Fill( itc*4 + tcdata.getTc(itc).address() );
  }
  
  if(ilp==0){
    if(inputbxId==3)
      for(int itc=0;itc<nTC;itc++) ((TH1D *) list->FindObject(Form("hTCEnergy_%d_%d_%d",ilp,imdl,int(tcdata.getTc(itc).address()))))->Fill( tcdata.getTc(itc).decodedE(tcdata.type()) );    
  }else{
    for(int itc=0;itc<nTC;itc++) ((TH1D *) list->FindObject(Form("hTCEnergy_%d_%d_%d",ilp,imdl,itc)))->Fill( tcdata.getTc(itc).decodedE(tcdata.type()) );    
  }
