...
int main(int argc, char** argv)
{
  ...
  std::map<uint64_t,std::vector<TPGFEDataformat::TcModulePacket>> econtemularray; //event,moduleId (emulation)
  std::map<uint64_t,std::vector<TPGFEDataformat::TcModulePacket>> econtarray; //event,moduleId (from link)
  ...
  void FillHistogram(TPGFEConfiguration::Configuration&,                                                             //to plot pedestal/threshold histograms
		     ...
		     const std::map<uint64_t,std::vector<TPGFEDataformat::TcModulePacket>>&,   //to plot the ECONT emulation results
		     const std::map<uint64_t,std::vector<TPGFEDataformat::TcModulePacket>>&,   //to plot the ECONT ASIC results or compare with the emulation
  ...
  for(int ieloop=0;ieloop<nloop;ieloop++){
	...
	const std::vector<TPGFEDataformat::TcModulePacket>& vecont = econtarray.at(event);
	for(const auto& modpair : vecont){
	  const uint32_t& modnum = modpair.first;
	  const TPGFEDataformat::TcRawDataPacket& econtlist = modpair.second;
	  econtlist.print();
	}//modules
...
// 		   const std::map<uint64_t,std::vector<TPGFEDataformat::TcModulePacket>>& econtemularray,   //to plot the ECONT emulation results
// 		   const std::map<uint64_t,std::vector<TPGFEDataformat::TcModulePacket>>& econtarray,   //to plot the ECONT ASIC results or compare with the emulation
