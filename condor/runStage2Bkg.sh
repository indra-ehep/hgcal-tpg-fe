...
###########################################################
outdir=$sampletype
outpath=ResultBkg_iter$iloop/$outdir
...
make clean
make
echo "stage2BkgRate.exe $infile $ofindex $nevents $sidelength $ofextn $sampletype \> out_${$sampletype}_${ofextn}_${clusproc}.log "
./stage2BkgRate.exe $infile $ofindex $nevents $sidelength $ofextn $sampletype > out_${sampletype}_${ofextn}_${clusproc}.log 2>&1
##########################################################
...
##########################################################
rsync -avP  TPGReco_tree_${ofextn}_${ofindex}.root $condorOutDir
